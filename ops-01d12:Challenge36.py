...
def banner_grab(address, port, tool):
    ...
    try:
        if tool == "netcat":
            ...
            result = subprocess.run(["nc", "-v", address, str(port)], capture_output=True, text=True)
            ...
            result = subprocess.run(["nmap", "-p", str(port), address], capture_output=True, text=True)
            # Extract relevant service information using regex (optional)
            match = re.search(r"(\d+)/tcp\s+open\s+(\S+)", result.stdout)
            if match:
                port, service = match.groups()
                return f"Nmap: Port {port} - {service}"
            else:
                return "Nmap: No service information found."
        else:
